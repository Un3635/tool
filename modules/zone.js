        ...
        Zone.prototype.extend({
            ...
            load: function(data) {
                if (data) {
                    this.data = data;
                    this.pageCount =
                        Math.ceil(this.data.length / this.cells) - 1;
                }
            ...
            nextPage: function() {
                return (
                    this.turnPage(this.page + 1) && this.fire('turn.nextpage')
                );
            },

            lastPage: function() {
                return (
                    this.turnPage(this.page - 1) && this.fire('turn.lastpage')
                );
            },
        ...
        $.each(
            {
                ...
                right: function(step, fn) {
                    if (
                        this.index % this.cols != this.cols - 1 &&
                        this.index + step <= this.pageSize
                    ) {
                        return fn.call(this, this.index + step);
                    }
                },
