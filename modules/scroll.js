    ...
    function($, module) {
        ...
        Scroll.prototype.extend({
            ...
            each: function(start) {
                ...
                if (start < 0 || start > len - l || this.start == start)
                    return this;
            ...
            move: function(step) {
                var i = this.index + step,
                    size = this.size - 1;

                if (i > size && this.start == this.data.length - this.size) {
                    return this.fire(
                        this.layout == 'y' ? 'over.down' : 'over.right'
                    );
                } else if (i < 0 && this.start == 0) {
                    return this.fire(
                        this.layout == 'y' ? 'over.up' : 'over.left'
                    );
                }
                ...
                if (i > size) {
                    var start = this.start + i - size,
                        last = this.data.length - this.size;
            ...
            fire: function(k, args) {
                ...
                var a = [k, this.index, this.data[dataIndex], dataIndex].concat(
                    args
                );
