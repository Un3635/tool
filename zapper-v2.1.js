;(function() {
    var version = '2.1', // 版本号
        expando = 'zapper' + (version + Math.random()).replace(/\D/g, ''); // 作为模块加载文件后缀标识
    ...
    function platform() {
        ...
        try {
            if (guangxi && guangxi.getStbNum) return 'ipanel.GX';
        } catch (e) {} // 茁壮广西中间件
        return 'pc';
    }
    ...
    function local() {
        var map = {
            'mozilla/4.0(compatible;msie6.0;eisipanel3.0;hi3716m)':
                'SH.songjiang',
    ...
    function Ajax(opt) {
        ...
        if (dataType === 'jsonp') {
            ...
            jsonpCallback =
                opt.jsonpCallback || 'jsonpCallback' + Date.parse(new Date());
            ...
            var events = [
                ...
                function(xhr) {
                    if (xhr.status === 200) {
                        deferred.resolve(
                            DataFactory(xhr.responseText, dataType),
                            xhr,
                            'success'
                        );
                    }
    ...
    Zloader.prototype.makeUrl = function(baseurl, targeturl) {
        ...
        return baseurl
            .slice(0, baseurl.length - targeturl.length)
            .concat(path)
            .join('/');
    };
    ...
    Zloader.prototype.getDependencies = function(key) {
        ...
        return function(name) {
            ...
            if (__this__.cache[modkey]) {
                ...
                __this__.cache[modkey] = m.callback(
                    $,
                    __this__.getDependencies(modkey)
                );
    ...
    function zapper(ids, callback) {
        ...
        if (ids && isObject(ids)) {
            for (var i in ids) {
                var url = (mod.dependencies[i] = zloader.makeUrl(
                    mod.url,
                    ids[i]
                ));
